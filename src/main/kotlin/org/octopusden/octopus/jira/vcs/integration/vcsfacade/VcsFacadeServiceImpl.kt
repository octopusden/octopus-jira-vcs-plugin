...
class VcsFacadeServiceImpl(
    ...
    override fun getSummary(issueId: Long): VcsFacadeService.IssueVcsSummary =
        try {
            with(
                vcsFacadeClient.findByIssueKeys(
                    issueManager.getAllIssueKeys(issueId).also {
                ...
                VcsFacadeService.IssueVcsSummary(
                    ...
                    VcsFacadeService.IssuePullRequestSummary(
                        pullRequests.size,
                        pullRequests.status?.let { pullRequestStatus ->
    ...
    override fun getCommits(issueId: Long): VcsFacadeService.Repositories<VcsFacadeService.Commit> =
        with(
            vcsFacadeClient.findCommitsWithFilesByIssueKeys(
                issueManager.getAllIssueKeys(issueId).also {
                            ...
                            VcsFacadeService.Commit(
                                ...
                                c.files.map { f ->
                                    VcsFacadeService.FileChange(
                                        VcsFacadeService.FileChange.Type.valueOf(f.type),
                                        f.link,
                                        f.path,
                                    )
                                },
    ...
    override fun getPullRequests(issueId: Long): Collection<VcsFacadeService.PullRequest> =
        with(
            vcsFacadeClient.findPullRequestsByIssueKeys(
                issueManager.getAllIssueKeys(issueId).also {
            ...
            this.map { pr ->
                VcsFacadeService.PullRequest(
                    ...
                    pr.reviewers
                        .map { r -> VcsFacadeService.Reviewer(r.user.name, r.user.avatar, r.approved) }
    ...
    override fun getBranches(issueId: Long): VcsFacadeService.Repositories<VcsFacadeService.Branch> =
        with(
            vcsFacadeClient.findBranchesByIssueKeys(
                issueManager.getAllIssueKeys(issueId).also {
