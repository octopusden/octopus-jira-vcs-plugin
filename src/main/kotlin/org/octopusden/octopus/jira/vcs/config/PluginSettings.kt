...
class PluginSettings
    ...
    constructor(
        ...
        operator fun get(setting: PluginProperty): Any = pluginSettings[setting.key] ?: getProperty(setting.key)
        ...
        fun getString(key: PluginProperty) = get(key) as? String ?: throw JiraApplicationException("Format '${key.key}' must be String")

        fun getList(key: PluginProperty) =
            getString(key)
                .split(",")
                .map(String::trim)

        fun getLong(key: PluginProperty) =
            getString(key)
                .toLong()
        ...
        ): Date = format.parse(getString(key))
